require(["jquery",
	...
	function selectWarnItem(warn, index) {
		...
		AppEvent.dispatchAppEvent(AppEvent.RELATE_QY_STATIONS, warn['enterprise_id']);
	...
	function init() {
		...
		var optionWarnDetailTable = {
			...
			uniqueId: 'station_id',
			onClickCell: function(field, value, row, $element) {
				AppEvent.dispatchAppEvent(AppEvent.POPUP_AND_CENTER_STATION, [row['station_id'], false]);
			},
			columns: [{
				title: '数据采集时间',
				field: 'warn_time',
