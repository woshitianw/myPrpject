...
public class ProxyServlet extends HttpServlet {
     ...
     */
    private final String stringProxyHost = PropertiesUtils.getProperties("proxy.url.host", "112.74.27.180");
    /**
     * The port on the proxy host to wihch we are proxying requests. Default value is 80.
     */
    private final int intProxyPort = PropertiesUtils.getInt("proxy.url.port", 8080);
    /**
     * The maximum size for uploaded files in bytes. Default value is 5MB.
     */
    private final int intMaxFileUploadSize = 5 * 1024 * 1024;
    ...
    public void doGet(HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse)
        ...
        LOGGER.debug("Proxy url: {}", httpServletRequest.getRequestURI());
        ...
        httpServletResponse.setHeader("Access-Control-Allow-Headers", "Content-Type,x-requested-with,Authorization,Access-Control-Allow-Origin");
        httpServletResponse.setHeader("Access-Control-Allow-Methods", "GET,OPTIONS");
        httpServletResponse.setHeader("Access-Control-Allow-Origin", "*");
        httpServletResponse.setHeader("Access-Control-Max-Age","360");
        this.executeProxyRequest(getMethodProxyRequest, httpServletRequest, httpServletResponse);
        stopWatch.stop();
        loggerProxyUrl(httpServletRequest.getRequestURI(), (int) stopWatch.getTime());
    }
...
//        httpServletResponse.setHeader("content-type", "text/html;charset=UTF-8");
        ...
        // 记录request请求参数
        LOGGER.info("Send the request to the target service, path = {}, headers = {}, params = {}", new Object[]{httpMethodProxyRequest.getPath(), JSON.toJSONString(httpMethodProxyRequest.getRequestHeaders()), JSON.toJSONString(httpMethodProxyRequest.getParams())});
        // Execute the request
        ...
        if (intProxyResponseCode >= HttpServletResponse.SC_MULTIPLE_CHOICES /* 300 */
            ...
            httpServletResponse.sendRedirect(stringLocation.replace(getProxyHostAndPort(), stringMyHostName));
            return;
    ...
    private String getProxyURL(HttpServletRequest httpServletRequest) {
        ...
        if(pathInfo.indexOf("weatherservice") != -1){
            stringProxyURL = "http://www.weather.tj.cn/DataService/json_swaquery";
        }
        else{
            stringProxyURL = "http://" + this.getProxyHostAndPort();
...
//        stringProxyURL += httpServletRequest.getRequestURI();
    ...
    private static void loggerProxyUrl(String url, int time) {
        LOGGER.info("Proxy http request complete, url = {}, time = {}", new Object[]{url, time});
    }
